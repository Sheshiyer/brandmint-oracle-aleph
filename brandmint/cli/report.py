...
def _fallback_summary_rows(report: ExecutionReport) -> list[dict]:
    """Extract unique provider fallback summaries from routing decision rows."""
    summaries: dict[tuple[str, str, str, str, str], dict] = {}
    for row in report.routing_decisions:
        if not isinstance(row, dict):
            continue
        attempts = row.get("fallback_attempts")
        provider_used = str(row.get("provider_used", "") or "")
        providers_tried = str(row.get("fallback_providers_tried", "") or "")
        fallback_order = str(row.get("fallback_order", "") or "")
        batch = str(row.get("batch", "") or "")
        if attempts in (None, "") and not provider_used and not providers_tried:
            continue
        key = (
            batch,
            provider_used,
...
def format_markdown(report: ExecutionReport) -> str:
    ...
    fallback_rows = _fallback_summary_rows(report)
    if fallback_rows:
        lines.extend([
            "## Provider Fallback Summary",
            "",
            f"- Recorded fallback summaries: {len(fallback_rows)}",
            "",
            "| Batch | Final Provider | Attempts | Providers Tried | Configured Order |",
            "|-------|----------------|----------|------------------|------------------|",
        ])
        for row in fallback_rows:
            lines.append(
                f"| {row.get('batch', '')} | {row.get('provider_used', '')} | "
                f"{row.get('attempts', '')} | {row.get('providers_tried', '')} | "
                f"{row.get('fallback_order', '')} |"
            )
        lines.append("")
...
<body>
    ...
    <h2>🔁 Provider Fallback Summary</h2>
    <p>Recorded: {len(fallback_rows)}</p>
    {"<table><tr><th>Batch</th><th>Final Provider</th><th>Attempts</th><th>Providers Tried</th><th>Configured Order</th></tr>" + "".join(f"<tr><td>{r.get('batch','')}</td><td>{r.get('provider_used','')}</td><td>{r.get('attempts','')}</td><td>{r.get('providers_tried','')}</td><td>{r.get('fallback_order','')}</td></tr>" for r in fallback_rows) + "</table>" if fallback_rows else ""}
...
def render_report_table(report: ExecutionReport):
    ...
    if fallback_rows:
        console.print(
            f"[bold]🔁 Provider fallback:[/] {len(fallback_rows)} batch summary row(s)"
        )
