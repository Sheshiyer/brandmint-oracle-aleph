...
## Image Frontmatter (All Pages)

Every wiki page may include visual asset references. These are populated from `wiki-asset-map.json` generated by `scripts/map-assets-to-wiki.py`.

```yaml
# Optional image fields for any page
hero_image: "/images/asset-filename.png"   # Full-width hero/banner image
images:                                     # Inline images referenced in body
  - path: "/images/asset-filename.png"
    alt: "Description of the image"
    section: "Section Name"                 # Which section this image belongs in
gallery:                                    # Gallery/carousel images
  - path: "/images/asset-filename.png"
    alt: "Description"
og_image: "/images/OG-IMAGE-filename.png"  # Open Graph social preview image
```

Agents should embed images in the markdown body using standard syntax:
```markdown
![Alt text](/images/filename.png)
```

Place hero images immediately after the H1 heading. Place inline images within their relevant sections.
...
Generated by Agent 6 (Visual Assets). Provides a complete gallery of all generated visual assets organized by category.

```yaml
---
title: "Visual Asset Library"
description: "Complete visual asset inventory for {BrandName}"
category: "brand"
tags: ["assets", "images", "visual", "gallery"]
lastUpdated: "{date}"
---
```
...
sections:
  - name: Identity Assets
    asset_ids: [2A, 2B, 2C]
    guidance: Brand kit, seal, logo emboss

  - name: Product Assets
    asset_ids: [3A, 3B, 3C]
    guidance: Product line, hero product, essence vial

  - name: Photography
    asset_ids: [4A, 4B]
    guidance: Catalog layout, flatlay compositions

  - name: Illustrations
    asset_ids: [5A, 5B, 5C]
    guidance: Heritage engraving, campaign grid, art panel

  - name: Icon System
    asset_ids: [5D-1, 5D-2, 5D-3]
    guidance: Regional traditions, life ceremonies, epic narratives

  - name: Narrative & Posters
    asset_ids: [7A, 8A, 9A, 10A, 10B, 10C]
    guidance: Contact sheet, posters, narrative sequences

  - name: Social & Digital
    asset_ids: [EMAIL-HERO, IG-STORY, OG-IMAGE, TWITTER-HEADER]
    guidance: Email heroes, social templates, OG images

format_per_asset: |
  ### {Asset Name} ({Asset ID})
  ![{Alt text}](/images/{filename})
  **Model:** {generation model}
  **Variants:** {count} versions available
```
